...
struct ContentView: View {
    ...
    var body: some View {
        ZStack {
            ...
            .frame(
                maxWidth: (
                    ((musicManager.isPlaying || !musicManager.isPlayerIdle) && viewModel.notchState == .closed && viewModel.showMusicLiveActivityOnClosed)
                    || (viewModel.expandingView.show && (viewModel.expandingView.type == .battery))
                    || Defaults[.inlineHudShow]) ? nil : viewModel.notchSize.width + ((hoverAnimation || (viewModel.notchState == .closed)) ? 20 : 0) + gestureProgress,
                maxHeight:
                    ((viewModel.sneakPeek.show && viewModel.sneakPeek.type != .music) || (viewModel.sneakPeek.show && viewModel.sneakPeek.type == .music && viewModel.notchState == .closed))
                    ? nil
            ...
            .frame(
                width: viewModel.notchState == .closed
                    ? (((musicManager.isPlaying || !musicManager.isPlayerIdle) && viewModel.showMusicLiveActivityOnClosed) || (viewModel.expandingView.show && (viewModel.expandingView.type == .battery)) || (Defaults[.inlineHudShow] && viewModel.sneakPeek.type != .music))
                    ? nil
        ...
        .shadow(
            color: (viewModel.notchState == .open && Defaults[.enableShadow] ? .black.opacity(0.6) : .clear),
