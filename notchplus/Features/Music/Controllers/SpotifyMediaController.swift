...
class SpotifyMediaController: MediaControllerProtocol {
    ...
    @Published private var playbackState: PlaybackState = PlaybackState(bundleIdentifier: "com.spotify.client")
    ...
    private func setupPlaybackStateChangeObserver() {
        notificationTask = Task { @Sendable [weak self] in
            ...
            for await _ in notifications { await self?.updatePlaybackInfo() }
        }
    }
    ...
    func next() async { await executeCommand("next track") }
    
    func previous() async { await executeCommand("previous track") }
